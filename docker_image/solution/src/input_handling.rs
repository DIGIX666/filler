...
pub fn input_handling() {
    ...
    loop {
        ...
        let grid = read_grid(&mut file);
        writeln!(file, "grid: {:?}", grid);

        // Lecture de la pièce.
        let piece = read_piece(&mut file);
...
\****************************************************/
fn read_grid(file: &mut std::fs::File) -> Vec<Vec<char>> {
    let mut input = String::new();
    io::stdin().read_line(&mut input).expect("Failed to read input");
    let grid_details = input.split_whitespace().collect::<Vec<&str>>()[2];
    let grid_lines = grid_details[..grid_details.len() - 1].parse::<i32>().unwrap();
...
\****************************************************/
fn read_piece(file: &mut std::fs::File) -> Vec<Vec<char>> {
    let mut input = String::new();
    io::stdin().read_line(&mut input).expect("Failed to read input");
